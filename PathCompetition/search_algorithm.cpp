...
void SearchAlgorithm::try_straight_successor(const Node &node, int delta_x, int delta_y, vector<Node> &succ) {
    xyLoc new_loc;
...
void SearchAlgorithm::try_diagonal_successor(const Node &node, int delta_x, int delta_y, vector<Node> &succ) {
    xyLoc new_loc;
    new_loc.x = node.xy_loc.x + delta_x;
    new_loc.y = node.xy_loc.y + delta_y;
